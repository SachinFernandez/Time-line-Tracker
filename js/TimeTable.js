...
class CalculationT{
    ...
    getTaskNames(shift){
        ...
        for(let key in shift){
            ...
            for(let key in indexObj){
                let nameObj = indexObj[key]['task_name'];
                    tasknames.push(nameObj);
                }
        }
        return tasknames;
    }
    getProjectNames(shift){
    let projectnames = [];
    // Access to Object rooted to Index Key
    for(let key in shift){
        let indexObj = shift[key];
        // Access to Object rooted to Name Key
        for(let key in indexObj){
            let nameObj = indexObj[key]['project_name'];
                projectnames.push(nameObj);
                }
        }
        return projectnames;
    }
    ...
    */
    getShiftTimeFromIndex(index,shift){
        ...
        for(let i in names){
            let array = names[i];
            let arrayObj = names[i]['entries'];
            for(let j in arrayObj){
                // Convert Array to String
                timeArray.push((Object.values(array['entries'][j]))[0]);
            }
        }
        return timeArray;
    }
    ...
    getIndexAndTime(shift){
        ...
        for(let key in shift){
            ...
            for(let name in indexObj){
                ...
                for(let i in colorObj['entries']){
                    let obj = colorObj['entries'][i];
        ...
        try{       
          ...
          // Access to Object rooted to Index Key
            for(let key in shift){
             ...
             // Access to Object rooted to Name Key
                for(let name in indexObj){
                    let nameObj = indexObj;
                    for(let key in nameObj){
                        let taskObj = nameObj[key]['task_name'];
                        let projObj = nameObj[key]['project_name'];
                        let timeObj = nameObj[key]['time'];
                        // Access to Object rooted to Color Key
                        for(let i in nameObj[key]['entries']){
                            let obj = nameObj[key]['entries'][i];
                        // To fetch as String, made another loop
                            for(let color in obj){
                                let time = obj[color];
...
class TimeTable{    // eslint-disable-line no-unused-vars
    constructor(data){
        ...
        this.v.shiftTime  = data["shift"];
        console.log(this.v.shiftTime);     // Time Table Data
...
class CanvasT extends CalculationT{
    ...
    */
    addTooltipEvent(shape, index, time, sc,shift){
        $(shape).on("click", ()=>{
            let shift = this.v.shiftTime;
            let subNames= this.getNames(this.v.shiftTime);
            let tasknames =this.getTaskNames(this.v.shiftTime);
            let projectnames =this.getProjectNames(this.v.shiftTime);
            //console.log(tasknames[index]);
            //console.log(projectnames[index]);
            //let taskTimes= this.getTaskTime(this.v.shiftTime);
            if(shift[index]){
            subNames = Object.keys(shift[index]).join("");
            tasknames = tasknames[index];
            projectnames = projectnames[index];
            console.log(subNames);
            console.log(tasknames);
            console.log(projectnames);
            }else{
            let selectBoxNames = this.v.option["selectBox"];
            subNames =  selectBoxNames[index];
            }
